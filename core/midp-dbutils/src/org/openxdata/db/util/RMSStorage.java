...
public class RMSStorage implements Storage{
	 ...
	 * @return - the list of objects retrieved.
	 */
	public Vector readSome(Class cls){
		try{
	...
	public boolean save(Record rec){
		if(rec.isNew()){
                        System.out.println("Mima New Record");
			int id = addNew(rec);
			if(id > 0)
				rec.setRecordId(id);
			return id > 0;
                    ...
                    System.out.println("Mima updated Record");
		...
		try{
			...
			if(open()){	
				int recId = recStore.getNextRecordID();
                                System.out.println("LOT REC ID:=>"+recId);
                                System.out.println("LOT NUM OF RECORDS :=>"+recStore.getNumRecords());
				if(recId > 0)
